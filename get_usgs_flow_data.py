...
from methods.processing.filter import filter_drb_sites, filter_usgs_nwis_query_by_type
...
gage_data_filename = f'{data_dir}/USGS/{boundary}_all_usgs_metadata.csv'
unmanaged_gauge_data_filename = f'{data_dir}/USGS/{boundary}_unmanaged_usgs_metadata.csv'
...
gage_data = gage_data[gage_data['years'] > 10]
...
for i, st in enumerate(gage_data.index):
    ...
    try:
        ...
        print(f'Got COMID for site {st} ({i} of {len(stations)})')
    except:
        gage_comid.loc[st, ['comid']] = np.nan
        print(f'Error getting COMID for site {st} ({i} of {len(stations)}')
...
tot_chars = nldi.getcharacteristic_byid(gage_data.comid, fsource = 'comid', 
                                        char_type= "tot", char_ids= tot_characteristics)
local_chars = nldi.getcharacteristic_byid(gage_data.comid, fsource = 'comid',
                                            char_type= "local", char_ids= cat_characteristics)
...
gage_with_cat_chars = gage_with_cat_chars.dropna(axis=0)
gage_with_cat_chars.index = gage_data.index
managed_stations = []
for i, st in enumerate(gage_data.index):
    total_management = gage_with_cat_chars.loc[st, tot_characteristics].sum()
    local_management = gage_with_cat_chars.loc[st, cat_characteristics].sum()
    if  (total_management + local_management) > 0:
...
for st in known_managed_sites:
    if st in unmanaged_gauge_data.index:
        unmanaged_gauge_data = unmanaged_gauge_data.drop(st)

# Get upstream gauges; this will save a JSON which can be used later
get_upstream_gauges(unmanaged_gauge_data.index, 
                    unmanaged_gauge_data, 
                    filename = 'upstream_gauges.json',
                    simplify=True)


# Export gage_data
gage_with_cat_chars.to_csv(f'./data/USGS/{boundary}_all_usgs_metadata.csv', sep=',')
...
print('Retrieving streamflow data for unmanaged gauges...')
nwis = NWIS()
