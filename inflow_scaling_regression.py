...
def train_inflow_scale_regression_models(reservoir, nyc_inflows):
    ...
    lrms = {q: sm.OLS(nyc_inflows[f'{reservoir} scaling'].loc[nyc_inflows['quarter'] == q],
                    sm.add_constant(np.log(nyc_inflows[f'{reservoir}_gages'].loc[nyc_inflows['quarter'] == q]))) for q in quarters}

    lrrs = {q: lrms[q].fit() for q in quarters}
    return lrms, lrrs


def predict_inflow_scaling(lrm, lrr, log_flow, method = 'random'):
    exog = lrm.exog
    exog[:,1] = log_flow
