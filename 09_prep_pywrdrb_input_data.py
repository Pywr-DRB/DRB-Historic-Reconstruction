...
from methods.prep_pywrdrb_inputs.disaggregate_DRBC_demands import disaggregate_DRBC_demands
from methods.prep_pywrdrb_inputs.extrapolate_NYC_NJ_diversions import extrapolate_NYC_NJ_diversions, download_USGS_data_NYC_NJ_diversions
from methods.prep_pywrdrb_inputs.predict_inflows_diversions import predict_inflows_diversions
...
PYWRDRB_INPUT_DIR = "./pywrdrb_inputs/"
ENSEMBLE_RECONSTRUCTION_DIR = OUTPUT_DIR + "ensembles/"

np.random.seed(SEED)

## Optional data processing steps
# Generally, these only need to be run once.  
# Assuming already run, set to False.
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        if download_flow_data_for_diversion_extrapolation:
            print("Downloading NYC & NJ diversion data.")
            download_USGS_data_NYC_NJ_diversions(dates=(f"{START_YEAR}-01-01", 
                                                        f"{END_YEAR}-12-31"))
        
        if extrapolate_diversions:
            print("Extrapolating NYC & NJ diversions.")
        ...
        if disaggregate_demands:
            print("Disaggregating DRBC demands.")
    ...
    if rank == 0:
        ...
        ensemble_filename = f"{ENSEMBLE_RECONSTRUCTION_DIR}catchment_inflow_{inflow_type}.hdf5"
